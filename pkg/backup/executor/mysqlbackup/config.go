...
// NewConfig creates an mysqlbackup configuration based on the input parameters.
...
func (c Config) Validate() (err error) {
	if c.username == "" {
		return fmt.Errorf("no mysqlbackup 'username' provided")
	}
	if c.password == "" {
		return fmt.Errorf("no mysqlbackup 'password' provided")
	}
	if c.databases == nil || len(c.databases) == 0 {
		return fmt.Errorf("no mysqlbackup 'databases' provided")
	}
	return nil
}
