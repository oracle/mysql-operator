...
type MySQLClusterInformer interface {
	...
	Lister() internalversion.MySQLClusterLister
}
...
func NewMySQLClusterInformer(client clientset_internalversion.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredMySQLClusterInformer(client clientset_internalversion.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Mysql().MySQLClusters(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Mysql().MySQLClusters(namespace).Watch(options)
			},
		},
		&mysql.MySQLCluster{},
		resyncPeriod,
...
func (f *mySQLClusterInformer) defaultInformer(client clientset_internalversion.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *mySQLClusterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&mysql.MySQLCluster{}, f.defaultInformer)
}

func (f *mySQLClusterInformer) Lister() internalversion.MySQLClusterLister {
	return internalversion.NewMySQLClusterLister(f.Informer().GetIndexer())
}
