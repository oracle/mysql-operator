...
type MySQLBackupLister interface {
	...
	List(selector labels.Selector) (ret []*mysql.MySQLBackup, err error)
...
// List lists all MySQLBackups in the indexer.
func (s *mySQLBackupLister) List(selector labels.Selector) (ret []*mysql.MySQLBackup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*mysql.MySQLBackup))
	})
	return ret, err
}
...
type MySQLBackupNamespaceLister interface {
	...
	Get(name string) (*mysql.MySQLBackup, error)
...
// List lists all MySQLBackups in the indexer for a given namespace.
func (s mySQLBackupNamespaceLister) List(selector labels.Selector) (ret []*mysql.MySQLBackup, err error) {
...
// Get retrieves the MySQLBackup from the indexer for a given namespace and name.
func (s mySQLBackupNamespaceLister) Get(name string) (*mysql.MySQLBackup, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(mysql.Resource("mysqlbackup"), name)
	}
	return obj.(*mysql.MySQLBackup), nil
}
