...
type MySQLBackupScheduleLister interface {
	...
	List(selector labels.Selector) (ret []*mysql.MySQLBackupSchedule, err error)
...
// List lists all MySQLBackupSchedules in the indexer.
func (s *mySQLBackupScheduleLister) List(selector labels.Selector) (ret []*mysql.MySQLBackupSchedule, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*mysql.MySQLBackupSchedule))
	})
	return ret, err
}
...
type MySQLBackupScheduleNamespaceLister interface {
	...
	Get(name string) (*mysql.MySQLBackupSchedule, error)
...
// List lists all MySQLBackupSchedules in the indexer for a given namespace.
func (s mySQLBackupScheduleNamespaceLister) List(selector labels.Selector) (ret []*mysql.MySQLBackupSchedule, err error) {
...
// Get retrieves the MySQLBackupSchedule from the indexer for a given namespace and name.
func (s mySQLBackupScheduleNamespaceLister) Get(name string) (*mysql.MySQLBackupSchedule, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(mysql.Resource("mysqlbackupschedule"), name)
	}
	return obj.(*mysql.MySQLBackupSchedule), nil
}
