...
type MySQLRestoreLister interface {
	...
	List(selector labels.Selector) (ret []*mysql.MySQLRestore, err error)
...
// List lists all MySQLRestores in the indexer.
func (s *mySQLRestoreLister) List(selector labels.Selector) (ret []*mysql.MySQLRestore, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*mysql.MySQLRestore))
	})
	return ret, err
}
...
type MySQLRestoreNamespaceLister interface {
	...
	Get(name string) (*mysql.MySQLRestore, error)
...
// List lists all MySQLRestores in the indexer for a given namespace.
func (s mySQLRestoreNamespaceLister) List(selector labels.Selector) (ret []*mysql.MySQLRestore, err error) {
...
// Get retrieves the MySQLRestore from the indexer for a given namespace and name.
func (s mySQLRestoreNamespaceLister) Get(name string) (*mysql.MySQLRestore, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(mysql.Resource("mysqlrestore"), name)
	}
	return obj.(*mysql.MySQLRestore), nil
}
