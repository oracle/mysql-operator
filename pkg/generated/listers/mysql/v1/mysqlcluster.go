...
// List lists all MySQLClusters in the indexer.
func (s *mySQLClusterLister) List(selector labels.Selector) (ret []*v1.MySQLCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.MySQLCluster))
	})
	return ret, err
}
...
// List lists all MySQLClusters in the indexer for a given namespace.
func (s mySQLClusterNamespaceLister) List(selector labels.Selector) (ret []*v1.MySQLCluster, err error) {
...
// Get retrieves the MySQLCluster from the indexer for a given namespace and name.
func (s mySQLClusterNamespaceLister) Get(name string) (*v1.MySQLCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("mysqlcluster"), name)
	}
	return obj.(*v1.MySQLCluster), nil
}
